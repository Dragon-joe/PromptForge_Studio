(() => {
  ...
  function renderQuickTemplates() {
    $('#quickTemplates').innerHTML = D.templates.slice(0, 4).map(template => `
      <button class="quick-item" data-template="${template.id}">
  ...
  function renderTemplates() {
    ...
    const list = D.templates.filter(t => {
      const haystack = `${t.title} ${t.titleAr} ${t.desc} ${t.descAr} ${t.category}`.toLowerCase();
    ...
    $('#templateGrid').innerHTML = list.length ? list.map(template => `
      <article class="template-card panel">
        <div class="template-top">
          ...
          <button class="favorite-btn ${favorites.includes(template.id) ? 'active' : ''}" data-favorite="${template.id}" title="Favorite">★</button>
        </div>
        ...
        <footer class="template-footer"><small>${template.popularity}% ${settings.language === 'ar' ? 'تطابق' : 'match'}</small><button class="primary-btn" data-template="${template.id}">${settings.language === 'ar' ? 'استخدام القالب' : 'Use template'}</button></footer>
  ...
  function handleCommandKeyboard(event) {
    ...
    items.forEach((item, i) => item.classList.toggle('active', i === state.commandIndex));
  ...
  function setStep(step) {
    ...
    $$('.builder-tab').forEach(tab => tab.classList.toggle('active', Number(tab.dataset.step) === state.step));
  ...
  function compilePrompt() {
    ...
    const json = { app: 'PromptForge Studio by Shark', version: APP_VERSION, generatedAt: new Date().toISOString(), project: form, sections };

    state.conflicts = detectConflicts(form);
    state.suggestions = detectSuggestions(form);
    state.quality = calculateQuality(form, state.conflicts, state.suggestions);
  ...
  function buildSections(form) {
    ...
    if (form.promptDepth === 'compact') {
      return Object.fromEntries(Object.entries(sections).filter(([name]) => ['SYSTEM ROLE','PRIMARY OBJECTIVE','FINAL OUTPUT','CONTENT FIDELITY','LANGUAGE AND ORGANIZATION','PROHIBITED ACTIONS','QUALITY ASSURANCE'].includes(name)));
    }
    return sections;
  }
  ...
  function detectConflicts(form) {
    const conflicts = [];
    const combined = `${form.primaryGoal} ${form.customRules.join(' ')}`.toLowerCase();
    const has = id => form.features.includes(id);
    const add = (level, title, detail) => conflicts.push({ level, title, detail });

    if (form.fidelity === 'strict' && form.taskType === 'egyptian-translation') add('danger', 'Strict preservation vs translation', 'Translation necessarily changes wording. Use Full Rewrite while preserving meaning, or keep the original beside the translation.');
    if (form.fidelity === 'strict' && /summari[sz]e|condense|rewrite|paraphrase|simplify|لخص|تلخيص|اختصر|إعادة صياغة|اعد صياغة|بسّط|بسط/.test(combined)) add('danger', 'Strict preservation vs rewriting', 'The prompt asks to preserve every word and also rewrite or condense. Put the rewritten material in a separate labelled section.');
    if (form.fidelity === 'strict' && form.languageMode === 'egyptian') add('danger', 'Strict wording vs Egyptian Arabic', 'Choose Preserve source language, or switch fidelity to Full Rewrite for translation.');
    if (form.outputFormat === 'PPTX' && ['word-template','strict-word'].includes(form.taskType)) add('danger', 'Task and output mismatch', 'A Word-template task cannot produce a PowerPoint file. Choose DOCX or change the task type.');
    if (form.outputFormat === 'DOCX' && form.taskType === 'presentation') add('warning', 'Presentation task with DOCX output', 'Choose PPTX unless the goal is only to write a slide outline in Word.');
    if (has('no-omission') && /summary only|one-page only|remove details|shorten drastically|ملخص فقط|صفحة واحدة فقط|احذف التفاصيل|اختصر بشدة/.test(combined)) add('danger', 'No omission vs aggressive shortening', 'Keep the full source, and place the short summary in an additional section.');
    if (has('no-external-facts') && /own expertise|external research|add facts|from the internet|من خبرتك|بحث خارجي|أضف معلومات|من الإنترنت|من الانترنت/.test(combined)) add('danger', 'Source-only vs external additions', 'Either remove the external-addition instruction or allow clearly labelled additions.');
    if (form.fidelity === 'enhance' && form.sourceOnly && !has('study-aid-box')) add('warning', 'Enhancement without separation', 'Enable a Study Aid box so added clarification stays visually separate from source content.');
    if (form.taskType === 'highlight-only' && form.fidelity !== 'strict') add('warning', 'Highlighting should preserve content', 'Selective highlighting normally uses Strict Preservation to avoid accidental rewriting.');
    if (questionsSelected(form) && form.fidelity === 'strict' && !/separate|appendix|additional|منفصل|ملحق|إضافي|اضافي/.test(combined)) add('warning', 'New questions with strict preservation', 'State that generated questions are an additional section and never replace original content.');
    if (form.outputFormat === 'PDF' && has('editable-elements')) add('warning', 'PDF editability limitation', 'PDF can preserve selectable content but is not as editable as DOCX. Use DOCX for full editability.');
    return conflicts;
  }
  ...
  function calculateQuality(form, conflicts, suggestions) {
    let score = 38;
    if (form.projectTitle) score += 7;
    if (form.primaryGoal.length >= 30) score += 13;
    else if (form.primaryGoal) score += 6;
    if (form.audience) score += 5;
    if (form.features.length >= 5) score += 8;
    if (form.features.length >= 10) score += 4;
    if (form.qaChecklist) score += 5;
    if (form.sourceOnly) score += 4;
    if (form.customRules.length) score += Math.min(5, form.customRules.length);
    if (form.features.some(id => D.prohibitedRules.some(item => item[0] === id))) score += 4;
    score -= conflicts.filter(c => c.level === 'danger').length * 14;
    score -= conflicts.filter(c => c.level === 'warning').length * 5;
    score -= Math.min(10, suggestions.length * 2);
  ...
  function renderProjects() {
    ...
    let projects = getProjects().filter(item => `${item.title} ${item.taskType}`.toLowerCase().includes(query));
  ...
  function handleProjectAction(action, id) {
    ...
    if (action === 'export') return download(`${slug(project.title)}.json`, JSON.stringify({ app:'PromptForge Studio by Shark', version:APP_VERSION, exportedAt:new Date().toISOString(), project }, null, 2), 'application/json');
  ...
  function handleVersionAction(action, projectId, versionId) {
    ...
    if (action === 'export') return download(`${slug(project.title)}_${slug(formatDate(version.createdAt))}.json`, JSON.stringify({ app:'PromptForge Studio by Shark', version:APP_VERSION, projectId, snapshot:version }, null, 2), 'application/json');
  ...
  function buildExportContent(format, form) {
    ...
    const meta = [
      ...
      `App: PromptForge Studio by Shark v${APP_VERSION}`
    ];
  ...
  function guideHtml(id) {
    ...
    const content = {
      ...
      settings: ar ? `<h2>الإعدادات</h2><p>يمكن تغيير اللغة والثيم وكثافة الواجهة واللون والحجم ومستوى الـPrompt الافتراضي.</p><h3>الحفظ التلقائي</h3><p>يحفظ المسودة الحالية بعد التعديلات. لا يتم رفعها لأي مكان.</p><h3>دمج التعليمات</h3><p>يحذف السطور المتكررة حرفيًا من الأقسام الناتجة لتقليل التكرار.</p>` : `<h2>Settings</h2><p>Configure language, theme, density, accent, interface size and default prompt depth.</p><h3>Autosave</h3><p>Saves the current draft after changes. Nothing is uploaded.</p><h3>Instruction deduplication</h3><p>Removes exact repeated lines from generated sections.</p>`,
  ...
  function exportDataObject() {
    ...
    const favorites = getStoredArray('favorites');
    return {
      app: 'PromptForge Studio by Shark',
      version: APP_VERSION,
      schemaVersion: 2,
  ...
  function sanitizeForm(form) {
    ...
    safe.pageSize = enumValue(safe.pageSize, ['A4','A3','Letter','16:9','Custom'], 'A4');
    ...
    safe.answerMode = enumValue(safe.answerMode, ['answers-only','answer-explanation','model-answer'], 'answer-explanation');
  ...
  function testStorage() {
    try { localStorage.setItem(PREFIX + '__test', '1'); localStorage.removeItem(PREFIX + '__test'); return true; } catch (_) { return false; }
  }
  ...
  function downloadDiagnosticLog() {
    const report = { app:'PromptForge Studio by Shark', version:APP_VERSION, generatedAt:new Date().toISOString(), userAgent:navigator.userAgent, settings, checks:state.diagnosticLog };
  ...
  function renderCommands(query = '') {
    ...
    const results = [...pages, ...actions, ...templates].filter(item => !normalized || `${item.title} ${item.subtitle}`.toLowerCase().includes(normalized)).slice(0,18);
    $('#commandResults').innerHTML = results.map((item,index) => `<button class="command-item ${index === state.commandIndex ? 'active' : ''}" data-command="${item.command}" data-value="${item.value}"><span>${item.icon}</span><div><b>${escapeHtml(item.title)}</b><small>${escapeHtml(item.subtitle)}</small></div></button>`).join('') || `<div class="empty-state">${langText('No matching command.','لا يوجد أمر مطابق.')}</div>`;
  }
