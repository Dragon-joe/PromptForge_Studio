...
const requiredIds = [
  ...
  'builderProgressBar','builderProgressText','promptStats','previousStepBtn','nextStepBtn','installAppBtn','updateBanner'
];
...
if (!Array.isArray(data.modelProfiles) || data.modelProfiles.length < 8) fail(`Expected at least 8 model profiles, found ${data?.modelProfiles?.length || 0}`);
const modelIds = data.modelProfiles.map(item => item.id);
if (new Set(modelIds).size !== modelIds.length) fail('Model profile ids must be unique.');
if (!modelIds.includes('gpt-5.6-sol')) fail('GPT-5.6 Sol profile is required.');
const sol = data.modelProfiles.find(item => item.id === 'gpt-5.6-sol');
if (sol?.modelId !== 'gpt-5.6-sol' || sol?.defaultReasoning !== 'high') fail('GPT-5.6 Sol profile is not configured correctly.');
if (!Array.isArray(data.executionModes) || data.executionModes.length < 5) fail(`Expected at least 5 execution modes, found ${data?.executionModes?.length || 0}`);
const executionIds = data.executionModes.map(item => item.id);
if (new Set(executionIds).size !== executionIds.length) fail('Execution mode ids must be unique.');
if (!Array.isArray(data.templates) || data.templates.length < 16) fail(`Expected at least 16 templates, found ${data?.templates?.length || 0}`);
...
if (!html.includes('Nexus Studio 5 by <span>Shark</span>')) fail('About attribution is missing.');
if (!html.includes('class="skip-link"') || !html.includes('role="tablist"') || !html.includes('role="progressbar"')) fail('Core accessibility structure is incomplete.');
if (!html.includes('ui-v5')) fail('Nexus Studio 5 interface class is missing.');
...
if (!app.includes('TARGET MODEL AND EXECUTION PROFILE') || !app.includes('renderReasoningOptions') || !app.includes("defaultModel: 'gpt-5.6-sol'")) fail('Model-aware prompt compilation is incomplete.');
if (!app.includes('calculateStepCompletion') || !app.includes('updatePromptStats') || !app.includes('trapFocus')) fail('Guided workflow or accessibility runtime is incomplete.');
if (!sw.includes(`promptforge-nexus-v${appVersion.split('.').slice(0,2).join('.')}`)) fail('Service Worker cache version does not match APP_VERSION.');
...
console.log(`Validation passed: v${appVersion}, ${data.modelProfiles.length} model profiles, ${data.templates.length} templates, ${idSet.size} unique ids, bilingual strings complete, no backend files.`);
